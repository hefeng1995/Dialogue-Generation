...
class Decoder(nn.Module):
    def __init__(self, args, de_vocab):
        ...
        if(args.rnn_cell == 'GRU'):
            ...
            self.infer_rnn = nn.GRU(input_size=3052,
                                    hidden_size=args.hidden_size,
                                    num_layers=args.layer_size,
                                    bias=True,
                                    dropout=args.drop_out
    ...
    def forward(self, decoder_input, h_decoder, h_infer):
        ...
        decoder_prob = F.softmax(decoder_logits, -1)
        decoder_prob_avg = F.avg_pool1d(decoder_prob.permute(
            1, 0, 2), self.avg_num).permute(1, 0, 2)
        infer_rnn_input = torch.cat(
            [decoder_prob_avg, decoder_output], -1)
    ...
    for epoch in range(1, epoch + 1):
        for step, batch in enumerate(train_iter):
            ...
            for index in range(sentence_len):
                decoder_step = F.cross_entropy(decoder_logits[index].view(-1, de_vocab_size),
                                               target[index].contiguous(
                ).view(-1), size_average=False,
                    ignore_index=pad)
                infer_step = F.cross_entropy(infer_logits[index].view(-1, de_vocab_size),
                                             target[index].contiguous(
                ).view(-1), size_average=False,
                    ignore_index=pad)
                infer_loss = infer_loss + infer_step
                decoder_loss = decoder_loss + infer_step.detach() * decoder_step
            ave_len = torch.sum(decoder_length.float())
            decoder_loss = decoder_loss / ave_len.float()
            infer_loss = infer_loss / ave_len.float()
            loss = decoder_loss + infer_loss
            loss.backward()
            ...
            if step % 100 == 0 and step != 0:
                ...
                print('---------------------------------')
                print("[%d][%d][total_step:%d][loss:%5.6f]" %
                      (epoch, step, len(train_iter), total_loss))
