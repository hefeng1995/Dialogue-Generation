...
def id2string(symbols, id2w_vocab):
    ...
    for batch_index in range(symbols.size(0)):
        all_str.append([id2w_vocab[id]
                        for id in symbols[batch_index]])
...
class Encoder(nn.Module):
    ...
    def forward(self, padded_encoder_inputs, encoder_input_len=0):
        ...
        # print(hidden_state.shape)
        hidden_state = torch.cat(hidden_state.chunk(2, -1), 0)
        # print(hidden_state.shape)
...
class Decoder(nn.Module):
    def __init__(self, args, de_vocab):
        ...
        if(args.rnn_cell == 'GRU'):
            ...
            self.decoder_rnn = SRU(input_size=args.embedding_size,
                                   hidden_size=args.hidden_size,
                                   num_layers=args.layer_size * 2,          # number of stacking RNN layers
    ...
    def forward(self, decoder_input, h_decoder):
        ...
        return decoder_logits, decoder_hidden_state


class Seq2Seq(nn.Module):
    def __init__(self, args, encoder, decoder, _go):
    ...
    def forward(self, encoder_inputs, encoder_length, decoder_inputs):
        _, decoder_hidden = self.encoder(encoder_inputs, encoder_length)
...
def train(epoch, model, optimizer, train_iter, en, de, args, test_iter=None):
    ...
    for epoch in range(1, epoch + 1):
        for step, batch in enumerate(train_iter):
            ...
            decoder_logits, decoder_symbols = model(
                encoder_inputs, encoder_length, decoder_inputs)
            # print(len(decoder_logits))
            # print(target.shape)
            optimizer.zero_grad()
            decoder_loss = F.cross_entropy(decoder_logits.view(-1, de_vocab_size),
                                           target.contiguous(
            ).view(-1),
                ignore_index=pad)
            loss = decoder_loss
...
def eval(model, data_iter, en, de, epoch_end=False):
    ...
    if(epoch_end):
        ...
        for step, batch in enumerate(data_iter):
            ...
            prediction_str = id2string(decoder_str, de.vocab.itos)
            total_bleu.append(cal_bleu(prediction_str, target_str))
        return np.mean(total_bleu)
    else:
        for batch in data_iter:
            ...
            prediction_str = id2string(decoder_str, de.vocab.itos)
            print('Bleu')
            print(cal_bleu(prediction_str, target_str))
            print('--------------------------')
